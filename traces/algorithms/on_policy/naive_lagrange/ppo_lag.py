...
from traces.adapter.onpolicy_adapter import OnPolicyCTAdapter, OnPolicyTraCeSAdapter, OnPolicyTraCeSHumanAdapter, OnPolicyCTHumanAdapter
from traces.algorithms import registry
from traces.classify.classifier import MujocoNPDataset, HumanNPDataset
from traces.common.buffer import VectorOnPolicyBufferH
from traces.models.actor_critic import ConstraintActorCriticH
from omnisafe.algorithms.on_policy.naive_lagrange.ppo_lag import PPOLag
from omnisafe.common.lagrange import Lagrange
from omnisafe.common.logger import Logger
from omnisafe.utils import distributed
from omnisafe.utils.config import Config
from traces.classify.classifier import classifier_nw_class, TrajDFData, collate, collate_maxlength, DistributionGRU, PtEstGRU
from traces.common.lagrange import LagrangeH
...
def _require_cv2():
    try:
        ...
        raise ImportError(
            'OpenCV is required for human-label/video workflows. '
...
class PPOLagCT(PPOLag):
    ...
    def _init_env(self) -> None:
        self._env: OnPolicyCTAdapter = OnPolicyCTAdapter(
            self._env_id,
...
class PPOLagTraCeS(PPOLag):
    ...
    def _init_env(self) -> None:
        self._env: OnPolicyTraCeSAdapter = OnPolicyTraCeSAdapter(
            self._env_id,
            self._cfgs.train_cfgs.vector_env_nums,
            self._seed,
            self._cfgs,
...
@registry.register
class PPOLagTraCeSHuman(PPOLagTraCeS):

    def _init_env(self) -> None:
        self._env: OnPolicyTraCeSHumanAdapter = OnPolicyTraCeSHumanAdapter(
            self._env_id,
            self._cfgs.train_cfgs.vector_env_nums,
            self._seed,
            self._cfgs,
...
@registry.register
class PPOLagCTHuman(PPOLagCT):

    def _init_env(self) -> None:
        self._env: OnPolicyCTHumanAdapter = OnPolicyCTHumanAdapter(
            self._env_id,
            self._cfgs.train_cfgs.vector_env_nums,
            self._seed,
            self._cfgs,
    ...
    def _init_model(self) -> None:
        ...
        super(PPOLagCT, self)._init_model()
...
# Backward-compatible algorithm names retained for old configs and scripts.
@registry.register
class PPOLagLearnedBC(PPOLagCT):
    pass


@registry.register
class PPOLagLearnedH(PPOLagTraCeS):
    pass


@registry.register
class PPOLagLearnedHuman(PPOLagTraCeSHuman):
    pass


@registry.register
class PPOLagLearnedBCHuman(PPOLagCTHuman):
    pass
