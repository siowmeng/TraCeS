...
from traces.classify.classifier import CostBudgetEstMLP, PtEstGRU, DistributionGRU, RLSFMLP
from traces.common.normalizer import NormalizerH, MeanNormalizer
...
from traces.envs.wrapper import (
    HiddenObsNormalize,
    TraCeSRewardNormalize,
    TraCeSActionScale,
    TraCeSUnsqueeze,
    get_wrapper_by_type
)

class OnPolicyCTAdapter(OnPolicyAdapter):
...
class OnPolicyTraCeSAdapter(OnPolicyAdapter):
    ...
    def _wrapper(
        ...
        if reward_normalize:
            ...
            # self._env = TraCeSRewardNormalize(self._env, device=self._device)
        if cost_normalize:
        ...
        self._eval_env = HiddenObsNormalize(self._eval_env, device=self._device,
                                            hidden_dim=self._cfgs.model_cfgs.classifier.hidden_dim,
                                            normalize=hidden_obs_normalize)
        self._env = TraCeSActionScale(self._env, low=-1.0, high=1.0, device=self._device)
        self._eval_env = TraCeSActionScale(self._eval_env, low=-1.0, high=1.0, device=self._device)
        if self._env.num_envs == 1:
            self._env = TraCeSUnsqueeze(self._env, device=self._device)
        self._eval_env = TraCeSUnsqueeze(self._eval_env, device=self._device)
...
class OnPolicyTraCeSHumanAdapter(OnPolicyTraCeSAdapter):
...
class OnPolicyCTHumanAdapter(OnPolicyCTAdapter):
...
# Backward-compatible adapter names retained for old imports and pickles.
OnPolicyLearnedBCAdapter = OnPolicyCTAdapter
OnPolicyLearnedHAdapter = OnPolicyTraCeSAdapter
OnPolicyLearnedHumanAdapter = OnPolicyTraCeSHumanAdapter
OnPolicyLearnedBCHumanAdapter = OnPolicyCTHumanAdapter
