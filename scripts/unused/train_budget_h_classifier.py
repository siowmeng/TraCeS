...
from ucrl.classify.classifier import mujoco_safety_gymnasium_dict, TrajNPData, MujocoNPDataset, CostBudgetHMLP, DistributionCostBudgetHMLP, collate

def main(env, list_npz_paths, modeldir,
         # storedatasetdir,
         logdir, testsplit, dropout, batchsize,
         # target_loss,
         target_accuracy, learn_rate, distribution_model, seed, deviceno):
    ...
    logfilename = (logdir + "/train" + ("_Distribution" if distribution_model else "_") + "BudgetH_"
    ...
    model_file_prefix = os.path.join(modeldir, today_str, env
                                     + ('_Distribution' if distribution_model else '_') +  'BudgetH_'
                                     + str(batchsize))
                                     # + str(gruunits) + '_' + str(grulayers) + '_' + str(batchsize))

    # obs, action, reward, cost, next_obs, done
    logger.info("Loading trajectories from NPZ files...")
    ...
    train_dataset = MujocoNPDataset(mujoco_domain=env, np_data=trajectories_data, indices=train_idx)
    train_dataloader = DataLoader(train_dataset, batch_size=batchsize,
                                  collate_fn=collate, shuffle=True)
    logger.info("Completed loading of train dataset...")

    logger.info("Loading test dataset...")
    test_dataset = MujocoNPDataset(mujoco_domain=env, np_data=trajectories_data, indices=test_idx)
    test_dataloader = DataLoader(test_dataset, batch_size=batchsize,
                                 collate_fn=collate, shuffle=True)
    logger.info("Completed loading of test dataset...")

    if distribution_model:
        net = DistributionCostBudgetHMLP(feature_dim=feature_dim, dropout=dropout).to(utils.device)
    else:
        net = CostBudgetHMLP(feature_dim=feature_dim, dropout=dropout).to(utils.device)
    ...
    while valid_accuracy < target_accuracy:
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            outputs, dict_log_c_out, h_out = net(inputs)
            # print(outputs.shape)
        ...
        for j, valid_data in enumerate(test_dataloader, 0):
            ...
            valid_outputs, dict_valid_log_c_out, valid_h_out = net(inputs_valid)
...
if __name__ == "__main__":
    ...
    main(args.env, args.list_npzfilepaths, args.modeldir,
         # args.storedatasetdir,
         args.logdir, args.testsplit, args.dropout, args.batchsize,
         # args.targetloss,
         args.targetacc, args.learn_rate, args.distribution_model, args.seed, args.deviceno)
